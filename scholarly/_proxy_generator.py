...
class ProxyGenerator(object):
    ...
    def Luminati(self, usr, passwd, proxy_port):
        ...
        if proxy_works:
            ...
            self.logger.warning(
                "Luminati does not seem to work. Reason unknown.")
        return proxy_works
    ...
    def SingleProxy(self, http=None, https=None):
        ...
        if proxy_works:
            ...
            self.logger.warning(
                "Unable to setup the proxy: http=%s https=%s. Reason unknown.", http, https)
        return proxy_works
    ...
    def Tor_External(self, tor_sock_port: int, tor_control_port: int, tor_password: str):
        ...
        self._can_refresh_tor, _ = self._refresh_tor_id(
    ...
    def _webdriver_has_captcha(self) -> bool:
        ...
        return self._has_captcha(
            ...
            lambda c: len(self._get_webdriver().find_elements(
                By.CLASS_NAME, c)) > 0,
        )
    ...
    def _get_webdriver(self):
        ...
        try:
            ...
            try:
                ...
                self.logger.info(
                    "Neither Chrome nor Firefox/Geckodriver found in PATH")
    ...
    def _get_chrome_webdriver(self):
        ...
        # options.add_argument('--headless')  # 注释掉以便在遇到 captcha 时显示浏览器窗口
        options.add_argument('--start-maximized')  # 最大化窗口，确保可见
        ...
        try:
            ...
            self._webdriver = webdriver.Chrome(service=Service(
                ChromeDriverManager().install()), options=options)
            # 如果没有 webdriver-manager，尝试使用系统 PATH 中的 chromedriver
    ...
    def _get_firefox_webdriver(self):
        ...
        # options.add_argument('--headless')  # 注释掉以便在遇到 captcha 时显示浏览器窗口
        # 使用 webdriver-manager 自动管理 GeckoDriver
        try:
            from selenium.webdriver.firefox.service import Service as FirefoxService
            from webdriver_manager.firefox import GeckoDriverManager
            self._webdriver = webdriver.Firefox(service=FirefoxService(
                GeckoDriverManager().install()), options=options)
        except ImportError:
            # 如果没有 webdriver-manager，尝试使用系统 PATH 中的 geckodriver
    ...
    def _handle_captcha2(self, url):
        print(f"\n请在弹出的浏览器窗口中手动完成验证码...{url}\n")
        cur_host = urlparse(self._get_webdriver().current_url).hostname

        # 尝试从 httpx session 获取 cookies 并添加到 WebDriver
        try:
            ...
            self.logger.debug(f"Could not add cookies from session: {e}")

        self._get_webdriver().get(url)

        # 等待页面加载完成
        self.logger.info("等待页面加载 10 秒...")
        time.sleep(10)

        # 首先确认页面上是否有 captcha
        initial_check = self._webdriver_has_captcha()
        self.logger.info(f"Initial captcha check: {initial_check}")

        if not initial_check:
            self.logger.warning(
                "No captcha detected on page. The page may have loaded successfully without captcha.")
            # 给用户一些时间查看页面，然后直接返回
            self.logger.info(
        ...
        self.logger.info(
            "Waiting for captcha to be solved. Please solve it manually in the browser window.")
        ...
        while cur < timeout:
            try:
                ...
                self.logger.info(
                    "Browser seems to be disfunctional - closed by user?")
                raise e
            except Exception as e:
    ...
    def _new_session(self, **kwargs):
        ...
        # 注释掉这行，避免在 captcha 处理期间关闭 webdriver
        # self._webdriver = None
    ...
    def _close_session(self):
        if self._session:
            self._session.close()
        # 只在明确需要时才关闭 webdriver
        # 不要在每次 _new_session 时自动关闭
        if self._webdriver:
            try:
    ...
    def ScraperAPI(self, API_KEY, country_code=None, premium=False, render=False):
        ...
        if (r["requestCount"] >= r["requestLimit"]):
            ...
            self.logger.warning(
                "ScraperAPI does not seem to work. Reason unknown.")
